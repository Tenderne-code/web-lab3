...
具体地，你需要将你的路由器实现为 C++ 类，我们将提供一个**单进程**模拟器用于测试，模拟器将会创建你实现的类对象，并**通过函数调用的方式模拟路由器收到报文并转发的过程**，因此，你不需要使用 socket 等方式进行网络通信。

## 1. 网络结构

在 Lab 3 中，我们考虑数据中心内部网络，并对网络拓扑进行了简化，如下图所示

![](net.png)

网络中若干路由器之间以任意方式连接成图。每个路由器只能有最多一个与外网连接的端口，其他端口均为内网端口（如图中 9.0.0.0/24 表示外部网络）。为了简单起见，每台 host 均有唯一的内网 IP 地址且只能连接一台路由器，而路由器不具有 IP 地址。每台路由器上可以有多个与其相连的 host。网络中存在一个用于 Lab 测试的 Controller 节点，它与每台路由器均相连。 

我们仅考虑路由器之间的连接权值，并将其余部分的权值均设为 0。 路由器之间的连接权值可以是任意非负整数。

**我们规定在 Lab 3 中的内网地址为 10.0.0.0/8, 所有其他地址均为外网地址**。
...
你实现的路由器类将用于创建每一台路由器上（即所有路由器的实现都是一致的）。总的来说，路由器需要支持在接收到一个报文后，根据目的地址决定转发到哪一个端口，另外需要实现网络地址转换 (NAT) 以支持内网对公网的访问。
...
4. 动态路由，即在网络拓扑变化的情况下能否正确计算得到最优路径

你可以自由设计 DV 算法的实现方式，我们将不会检查路由器上的距离向量表以及交换的 `DV` 报文。 

### 2.2. NAT

另一方面，对于存在外网端口的路由器，它需要支持最基础的 NAT 功能。在 Lab 3 中，为了简单起见， NAT 的工作方式与课堂上存在差别，我们将直接基于 IP 地址进行映射，即将内网 IP 直接映射为一个外网 IP ，而不基于 IP:port 进行映射。

因此， Lab 3 中的 NAT 功能包括：

1. 每个路由器需支持配置最多 256 个可用公网地址用于 IP 映射。
2. 每个路由器支持最多一个外网端口。
3. 对于来自内网端口的报文，若下一跳将从外网端口发出，需要执行网络地址转换，即若源地址尚未分配公网地址，则需要为该源地址分配一个可用公网地址，并将报文中的源地址替换为对应公网地址后再发出。若已分配地址，则直接将源地址转换为对应公网地址后发出。若没有空闲的可用公网地址，则丢弃该报文。
4. 对于来自外网端口的报文，若其目的地址不是该路由器在 2 中已分配的公网地址，则直接丢弃。否则将目的地址转换为对应的内网地址后再进行路由转发。

**注意，本 Lab 中并没有对连接状态进行检查。**

**我们认为路由器的外网地址范围中的 ip 都是可达的，请注意处理从内网访问外网的报文**。
...
你可以自由选择 NAT 的实现方式，在本 Lab 中，你只应该在收到来自 Controller 的 `RELEASE NAT ITEM` 报文时释放可用公网地址，我们将在 3. 中解释这一问题。
...
**你不需要实现 Controller**，但为了进行测试，你需要能够处理来自 Controller 的控制报文，并作出相应的操作。 Controller 的报文将和其他报文一样通过 Simulator 以函数调用的方式传递给路由器。
...
对于 `TYPE_DATA` ，其后的 payload 仅包含数据。路由器应当转发该种报文，并且保持 payload 不变，**测试时将会检查 payload** 。
...
**注意，单个报文的长度不应该超过 8192（报头+payload）**
...
该指令格式为 `<type> <port> <value> <ip>` ，其中 `<port>` , `value` 为整数值， `ip` 为一个点分十进制表示的地址，一条合法的指令例如 `3 2 10 114.114.114.114` 。该条指令通知路由器在 `<port>` 端口上连接了一台 IPv4 地址为 `<ip>` 的 host，链路 cost 为 `<value>` 。路由器收到该指令后应该更新路由。
...
+ `external_addr` 表示该路由器的外网端口对应的地址范围，其使用 CIDR 进行表示，例如 `177.0.0.0/24` 表示的地址范围为 `177.0.0.0` 到 `177.0.0.255` 
+ `available_addr` 表示该路由器的可用公网地址范围，表示方式同 `external_addr` 

简单起见，我们约定 `external_addr` 和 `available_addr` 最多包含 256 个地址。
...
你可以在 `RouterBase` 规定的框架下以自己喜欢的方式实现 `router_init()` 和 `router()` ，但你的实现需要考虑到模拟器对 `RouterBase` 的调用方式。

Simulator 通过创建多个 router 实例和一个 Controller 实例来模拟实机环境。为了使 Simulator 能够创建你定义的 router 类，我们在 `router_prototype.h` 中声明了一个辅助函数
...
Simulator 将首先通过 `create_router_object` 创建 router 对象，紧接着调用 `router_init` 完成初始化。

Sumulator 和 Controller 在测试时将基于如下逻辑运行：

``` cpp
test()
{
    # simulator creates controller
    # simulator craetes router objects and call router_init()
    # controller generates some control packet
    # simulator sends packets to routers and forwards returned packet by router() function call
    # simulator waits until no packet is forwarding or packet forwarded too many times
    # tester generates some data packet
    # simulator traces data packet forwarding 
    # tester check if data packets is correctly forwarded
}
```

**注意，所有网络拓扑结构将完全通过 control packet 进行定义。所有报文都将通过函数调用被传递给 router 或被 router 返回。**
...
你可以认为测试时 Simulator 将主要存在两种操作，修改和查询。这里的修改指通过 Controller 发送 control packet 定义或修改网络结构，查询指发送 data packet 并追踪转发路径以检查正确性。
**测试程序保证在发生修改后的查询开始前 Simulator 会通过 Controller 向所有路由器发送 `TRIGGER DV SEND` 报文，并持续转发报文，直到网络中不再存在报文或所有路由器的转发次数超过限制后再开始查询**。

**我们将单次向所有路由器发送 TRIGGER DV SEND 后 Simulator 允许的转发次数设定为了一个足够大的数值，我们认为一个正确实现的 DV 算法不会超过该限制。**
...
1. `new <port_num> <external_port> <external_addr> <available_addr>` 创建一个新的路由器，参数与 `router_init()` 的参数一致， `simulator` 将会返回创建的路由器的编号（正整数） 。该指令将会创建 router 实例并调用 `router_init()` 
