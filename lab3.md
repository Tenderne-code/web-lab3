...
该指令格式为 `<type> <port> <value>` ，其中 `<port>` 为正整数, `<value>` 为整数，一条合法的指令例如 `2 5 6` 。该条指令意为将路由器的 `<port>` 端口的权值更新为 `<value>` 。路由器在收到该指令后，应当更新端口所对应链路的权值。
...
**测试程序保证在第 3 步中 Controller 会在控制报文的最末尾生成一条发往所有路由器的 `TRIGGER DV SEND` 报文，用于强制路由器开始一次距离向量交换，对于实现正确的 DV 算法，这足以保证在第 6 步开始前 DV 算法收敛到最优值**。
